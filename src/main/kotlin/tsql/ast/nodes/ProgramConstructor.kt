...
class ProgramConstructor(val syntaxErrorListener: SyntaxErrorListener) : TSQLParserBaseVisitor<ProgramAST>() {
    ...
    override fun visitProgram(ctx: TSQLParser.ProgramContext): ProgramAST {
        ...
        try {
            program = ProgramAST(position = Pair(Pair(0, 0), Pair(ctx.EOF().symbol.line, ctx.EOF().symbol.charPositionInLine + ctx.EOF().symbol.text.length)))
        } catch (e: Exception) {
            program = ProgramAST()
        }
        ...
        for (statement in ctx.coalesce_statement()) {
            try {
                program.statementList.add(statement.accept(CoalesceStatementConstructor(syntaxErrorListener)))
            } catch (e: Exception) {
                println(e)
            }
        }
